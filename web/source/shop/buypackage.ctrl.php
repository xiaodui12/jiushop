...
$usergroups=pdo_fetchall("SELECT * FROM".tablename('users_group') ."where price >= 1 AND  price != 1001 order by price asc",array(':price'=>$group['price']),'id');
	if($_GPC['groupid']){
		...
		if(empty($usergroups[$groupid])){message('访问错误',url('shop/buypackage'),error);}
		if($groupid == $group['id']){
			$credit  = $user['credit2'] - $group['price'];
			$endtime = $user['endtime'] + 365*86400;
			if($credit<0){message('余额不足，您当前还需充值'.(-$credit).'元方能成功续费',url('shop/member'),error);}
			else{
				...
				message('续费成功！',url('shop/buypackage'),success);
			}
		}
		else{
			$price  = sprintf("%.2f",($usergroups[$groupid]['price'] - $group['price'])*($user['endtime']- TIMESTAMP)/31536000);
			$credit = $user['credit2'] - $price;
			if($credit<0){message('余额不足，您当前还需充值'.(-$credit).'元（计算公式：(剩余天数/365)*套餐差价）方能成功升级套餐',url('shop/member'),error);}
			else{
				...
				message('升级成功！',url('shop/buypackage'),success);
			}
		}
	}
}
