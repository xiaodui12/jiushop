...
class Credit_EweiShopV2Page extends WebPage
{
	protected function main($type = 'credit1')
	{
		...
		if ($_GPC['groupid'] || $_GPC['level'] || $_GPC['keyword']) {
			...
			if ($type == 'credit1') {
				$table1 = "select log.id,log.num,log.presentcredit,log.createtime,log.remark,log.credittype,m.id as mid,m.openid, m.realname,m.nickname,m.avatar, m.mobile, m.weixin,u.username,g.groupname,l.levelname from " . tablename("mc_credits_record") . " log " . " left join " . tablename("users") . " u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid" . " left join " . tablename("ewei_shop_member") . " m on m.uid=log.uid" . " left join " . tablename("ewei_shop_member_group") . " g on m.groupid=g.id" . " left join " . tablename("ewei_shop_member_level") . " l on m.level =l.id" . " where 1 " . $condition . " and log.uid<>0";
			...
			$table1 = "select log.id,log.num,log.presentcredit,log.createtime,log.remark,log.credittype,u.username,log.uid,'xxx' as openid from " . tablename("mc_credits_record") . " log " . " left join " . tablename("users") . " u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid" . " where 1 " . $condition . " and log.uid<>0";
