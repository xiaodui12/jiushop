...
class Order_EweiShopV2Page extends PluginWebPage
{
    protected function orderData($status, $st)
    {
        ...
        if($rebate_status!="all"){
            ...
            $over_time=time()-intval($set["rebatetime"])*60*60*24;
            if($rebate_status==1){
                $condition.=" and og.rebate_order_status=0 and o.sell_status=0 ";
            }elseif ($rebate_status==2){
                $condition.=" and og.rebate_order_status=1 and o.sell_status=0 and og.rebate_order_time>".$over_time;
            }elseif ($rebate_status==3){
                $condition.=" and (o.sell_status=1 or  (  og.rebate_order_status=1 and og.rebate_order_time<".$over_time."))";
            }
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)) {
            $sql = 'select o.* ,og.rebate_order_status,og.rebate_order_time from ' . tablename('ewei_shop_order') . ' o left join ims_ewei_shop_order_goods as og on og.orderid=o.id' . (' ' . $sqlcondition . ' where ' . $condition . ' ' . $statuscondition . ' GROUP BY o.id ORDER BY o.createtime DESC  ');
        ...
        $send_time=time()-intval($set["rebatetime"])*60*60*24;
        ...
        if (!empty($list)) {
            foreach ($list as &$value) {
                ...
                $value['status']=$value["rebate_order_status"]==0?"挂售中":"已售出";

                if(($value["rebate_order_status"]==1&&intval($value['rebate_order_time'])<$send_time)||$value["sell_status"]==1)
                {
                    $value['status']="已完成";
                }
